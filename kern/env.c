...
env_setup_vm(struct Env* e) {
    ...
    memcpy(e->env_pgdir + PDX(UTOP), kern_pgdir + PDX(UTOP), 4096 - PDX(UTOP) * 4);
...
region_alloc(struct Env* e, void* va, size_t len) {
    ...
    uintptr_t modified_start = (uintptr_t) va;
    size_t modified_end = 0;
    modified_start = ROUNDDOWN(modified_start, PGSIZE);
    modified_end = modified_start + ROUNDUP(len, PGSIZE);

    for (uintptr_t i = modified_start; i < modified_end; i += PGSIZE) {
        ...
        struct PageInfo* new_ptf = page_alloc(!ALLOC_ZERO);

        new_ptf->pp_ref++;
        // struct PageInfo* new_ptf = &new_ptf->pp_ref++;

        *pte = (physaddr_t) (page2pa(new_ptf) | PTE_U | PTE_W | PTE_P);
    }
}
...
void env_pop_tf(struct Trapframe* tf) {
    asm volatile(
        "\tmovl %0,%%esp\n" //把tf指向地址赋值给esp，改变栈指针地址，就是为了下面的恢复程序的上一次运行的状态
        "\tpopal\n"
        "\tpopl %%es\n"
        "\tpopl %%ds\n"
